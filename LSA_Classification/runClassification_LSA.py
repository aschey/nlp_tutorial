...
for k in range(1,10):
    ...
    knn_tfidf = KNeighborsClassifier(n_neighbors=k, algorithm='brute', metric='cosine')
    ...
    print("  k = %d (%d / %d) correct - %.2f%%" % (k, numRight, len(y_test), float(numRight) / float(len(y_test)) * 100.0))
    ...
    # Build a k-NN classifier. Use k = 5 (majority wins), the cosine distance, 
    # and brute-force calculation of distances.
    knn_lsa = KNeighborsClassifier(n_neighbors=k, algorithm='brute', metric='cosine')
